...
import (
	...
	"github.com/denirjr/gocourse/escrever_arquivos/model"
)
...
func main() {
	...
	if err != nil {
		fmt.Println("[main]  Houve um erro ao ler o arquivo com leitor csv. Erro",
			err.Error())
		return
	}
	...
	if err != nil {
		fmt.Println("[main] Houve um erro ao criar o arquivo json. Erro",
			err.Error())
		return
	}
	...
	for _, linha := range conteudo {
		for indiceItem, item := range linha {
			...
			if err != nil {
				fmt.Println("[main]  Houve um erro ao gerar o json do item ", ". Erro: ",
					err.Error())
			}
